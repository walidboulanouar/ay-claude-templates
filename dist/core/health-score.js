 ...
 */
export async function calculateHealthScore(packageData, apiClient) {
    const scores = {
        maintenance: 0,
    ...
    const overall = scores.maintenance * 0.3 +
        scores.security * 0.3 +
 ...
 */
export function displayHealthScore(score) {
 ...
 */
export async function getSmartRecommendations(apiClient, limit = 5) {
    ...
    const filtered = recommendations.filter((item) => !installedIds.has(`${item.slug}-${item.type}`));
