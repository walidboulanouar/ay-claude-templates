...
export async function installCommand(packages, options) {
    ...
    for (const pkgName of packages) {
        ...
        try {
                ...
                const { selected } = await inquirer.prompt([
                    {
                        ...
                        choices: searchResults.items.map((item) => ({
            ...
            const downloadResponse = await apiClient.client.post(`/functions/v1/download-${selectedPackage.type}`, {
            ...
            const verification = await PackageVerifier.verifyPackage(tempZip, tempZip, // Will extract to verify
            selectedPackage.type, downloadInfo.integrityHash);
            ...
            await recordVersionInstallation(selectedPackage.id, selectedPackage.slug, selectedPackage.type, downloadInfo.version, scope);
